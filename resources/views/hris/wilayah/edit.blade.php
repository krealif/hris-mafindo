@extends('layouts.dashboard', [
    'title' => "Edit {$branch->name} | Wilayah",
])
    ...
    <div class="container-xl">
      <div class="row g-3">
        <div class="col-12 col-md-10 col-lg-6">
          ...
          @endif
          <form class="card card-mafindo" method="POST" action="{{ route('wilayah.update', $branch->id) }}">
            @csrf
            @method('PATCH')
            <div class="card-body">
              <div class="mb-3">
                <label for="name" class="form-label required">Nama Wilayah</label>
                <x-form.input name="name" type="text" value="{{ old('name', $branch->name) }}" required />
              </div>
            </div>
